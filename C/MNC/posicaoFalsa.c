...
int posicao (funcao termo, double a, double b, double erro, int it_max, double *raiz, int *it_efetuada){
    int i; 
    double x, d = 2;
    if(resultaFuncao(termo, a) * resultaFuncao(termo, b) < 0){
        for(i = 0; i < it_max; i++){
            x = (a * resultaFuncao(termo, b) - b * resultaFuncao(termo, a)) / (resultaFuncao(termo, b) - resultaFuncao(termo, a));
...
int main(){
    ...
    printf("%lf\n", precisao);
    if(posicao(fx, a, b, precisao, it_max, &raiz, &it_efetuada))
        printf("Resultado = %lf\nIteracoes = %d\n", raiz, it_efetuada);
    else
