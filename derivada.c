...
double derivada_primeira (funcao termo, double valor, double precisao, int it_max, int *it_efetuada){
    int i; 
    double fx, ant, h = 1, erro_ant, d = 2;
    for(i = 0; i < it_max; i++){
        if(i == 0){
            fx = (resultaFuncao(termo, valor + h) - resultaFuncao(termo, valor - h)) / (d * h);
        }
        else if(i == 1){
            fx = (resultaFuncao(termo, valor + h) - resultaFuncao(termo, valor - h)) / (d * h);
            ...
            fx = (resultaFuncao(termo, valor + h) - resultaFuncao(termo, valor - h)) / (d * h);
            /*printf("%lf - %lf / 2 * %lf = %lf\n", resultaFuncao(termo, valor + h), resultaFuncao(termo, valor - h), h, fx);
            printf("%lf * %lf = %lf\n", d, h, d * h);
...
int main(){
    ...
    if(valor = derivada_primeira(fx, valor, precisao, it_max, &it_efetuada))
        printf("Resultado = %lf\nIteracoes = %d\n", valor, it_efetuada);
    else
